...
public class LinkDaoImpl implements LinkDao {
	...
	private String buildQueryByProperties(
			Map<String, List<Integer>> multipleValueProperties,
		...
		for(String m: multipleValueProperties.keySet()) {
			if (multipleValueProperties.get(m) != null) {
				...
				//if at least one multiple value property is not null, the "where" clause is present in the query
				isWherePresent = true;
			} 
		}
		
		if(isWherePresent) {
			//there is already a where clause, we just need to conjunct another set of conditions
			queryString.append(" and");
		} else {			
			//as there were no multiple value properties, the where clause is missing
	...
	private TypedQuery<Link> setSearchParameters(
			Map<String, List<Integer>> multipleValueProperties,
			Map<String, String> singleValueProperties, 
