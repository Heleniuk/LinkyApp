...
<body>
	<ul>
		<c:forEach var="child" items="${nodes}">
		    <li>
		    	<span>		    		
		    		<c:url var="deleteUrl" value="/categories/delete/${child.id}"/>
					<c:url var="editUrl" value="/categories/edit/${child.id}"/>	
		    		<cf:form method="POST" action="${deleteUrl}">
		    			<i class="glyphicon glyphicon-menu-up"></i>
					    <label class="repellable-left">${child.title}    
		    	...
		    	<ul>
				    <c:forEach var="link" items="${child.links}">					
						<li>
							<a class="label label-success" href="${link.linkAddress}">		
